...
goog.provide('Blockly.Franzininho.text');
 ...
 */
Blockly.Franzininho['text'] = function(block) {
  var code = Blockly.Franzininho.quote_(block.getFieldValue('TEXT'));
 ...
 * @return {array} Completed code with order of operation.
 */
Blockly.Franzininho['text_join'] = function(block) {
  var code;
  if (block.itemCount_ == 0) {
    return ['""', Blockly.Franzininho.ORDER_ATOMIC];
  } else if (block.itemCount_ == 1) {
    var argument0 = Blockly.Franzininho.valueToCode(block, 'ADD0',
        Blockly.Franzininho.ORDER_UNARY_POSTFIX) || '""';
    ...
    for (var n = 0; n < block.itemCount_; n++) {
      argument = Blockly.Franzininho.valueToCode(
          block, 'ADD' + n, Blockly.Franzininho.ORDER_NONE);
      if (argument == '') {
        code[n] = '""';
      } else {
 ...
 */
Blockly.Franzininho['text_append'] = function(block) {
  // Append to a variable in place.
  var varName = Blockly.Franzininho.variableDB_.getName(
      block.getFieldValue('VAR'), Blockly.Variables.NAME_TYPE);
  var argument0 = Blockly.Franzininho.valueToCode(block, 'TEXT',
      Blockly.Franzininho.ORDER_UNARY_POSTFIX);
  if (argument0 == '') {
    argument0 = '""';
  } else {
 ...
 * @return {array} Completed code with order of operation.
 */
Blockly.Franzininho['text_length'] = function(block) {
 ...
 * @return {array} Completed code with order of operation.
 */
Blockly.Franzininho['text_isEmpty'] = function(block) {
  var func = [];
  func.push('boolean ' + Blockly.Franzininho.DEF_FUNC_NAME + '(String msg) {');
  ...
  func.push('  }');
  func.push('}');
  var funcName = Blockly.Franzininho.addFunction('isStringEmpty', func.join('\n'));
  var argument0 = Blockly.Franzininho.valueToCode(block, 'VALUE',
      Blockly.Franzininho.ORDER_UNARY_POSTFIX);
  if (argument0 == '') {
    argument0 = '""';
 ...
 * @return {array} Completed code with order of operation.
 */
Blockly.Franzininho['text_trim'] = function(block) {
  // Trim spaces.
  Blockly.Franzininho.text_trim.OPERATORS = {
    LEFT: '.trim()',
    RIGHT: '.trim()',
    BOTH: '.trim()'
  };
  var mode = block.getFieldValue('MODE');
  var operator = Blockly.Franzininho.text_trim.OPERATORS[mode];
  var argument0 = Blockly.Franzininho.valueToCode(block, 'TEXT',
      Blockly.Franzininho.ORDER_UNARY_POSTFIX);
  if (argument0 == '') {
    argument0 = '""';
  ...
  return [argument0 + operator, Blockly.Franzininho.ORDER_UNARY_POSTFIX];
};
 ...
 * @return {string} Completed code.
 */
Blockly.Franzininho['text_print'] = function(block) {
  var serialId = Blockly.Franzininho.Boards.selected.serial[0][1];
  var setupCode = serialId + '.begin(9600);';
  Blockly.Franzininho.addSetup('serial_' + serialId, setupCode, false);
  var argument0 = Blockly.Franzininho.valueToCode(block, 'TEXT',
 ...
 * @return {array} Completed code with order of operation.
 */
Blockly.Franzininho['text_prompt_ext'] = function(block) {
  ...
  if (toNumber) {
    func.push('int ' + Blockly.Franzininho.DEF_FUNC_NAME + '(String msg) {');
  } else {
    func.push('String ' + Blockly.Franzininho.DEF_FUNC_NAME + '(String msg) {');
  }
  ...
  func.push('}');
  var funcName = Blockly.Franzininho.addFunction(
  ...
  var msg = Blockly.Franzininho.valueToCode(block, 'TEXT',
      Blockly.Franzininho.ORDER_NONE) || '""';
 ...
 * ***************************************************************** */
Blockly.Franzininho['text_endString'] = function(block) {
  return ['', Blockly.Franzininho.ORDER_UNARY_POSTFIX];
};

Blockly.Franzininho['text_indexOf'] = function(block) {
  return ['', Blockly.Franzininho.ORDER_UNARY_POSTFIX];
};

Blockly.Franzininho['text_charAt'] = function(block) {
  return ['', Blockly.Franzininho.ORDER_UNARY_POSTFIX];
};

Blockly.Franzininho['text_getSubstring'] = function(block) {
  return ['', Blockly.Franzininho.ORDER_UNARY_POSTFIX];
};

Blockly.Franzininho['text_changeCase'] = function(block) {
  return ['', Blockly.Franzininho.ORDER_UNARY_POSTFIX];
};

Blockly.Franzininho['text_prompt'] = function(block) {
  return ['', Blockly.Franzininho.ORDER_UNARY_POSTFIX];
};
