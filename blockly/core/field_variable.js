...
/**
 * OVER RIDE
 * Add another variable to the constructor to allow only that the block is created with
 * to be in the drop down.  The reason for this feature is that if you are using a type
 * array you would not want the block that sets that array to only have that variable.  So you
 * never get 2 blocks setting the same array.
 *
 * Class for a variable's dropdown field.
 * @param {?string} varname The default name for the variable.  If null,
 *     a unique variable name will be generated.
 ...
 * @param {string=} opt_defaultType The type of variable to create if this
 *     field's value is not explicitly set.  Defaults to ''.
 * @param {boolean=} opt_showOnlyVariableAssigned shows only the first field assigned to it
 * @param {boolean} opt_createNewVariable
 * @extends {Blockly.FieldDropdown}
 * @constructor
 */
Blockly.FieldVariable = function(varname, opt_validator, opt_variableTypes,
...
/**
 * Over Riding to Include the options available
 *
 * Construct a FieldVariable from a JSON arg object,
 * dereferencing any string table references.
 * @param {!Object} options A JSON object with options (variable,
